...
# =============================
st.set_page_config(
    page_title="GARV – Airport Operations Intelligence",
    page_icon="✈️",
    layout="wide",
...
.sugg-row { display:flex; gap:8px; flex-wrap:wrap; margin-top:10px; }
...
# =============================
# SUGGESTED NEXT QUESTIONS (UI helper)
# =============================
def render_suggested_questions(out: Dict[str, Any], chat: Dict[str, Any]) -> None:
    ...
    st.markdown("<div class='section-title'>Suggested next questions</div>", unsafe_allow_html=True)
    st.caption("Click to run a related follow-up using the same context.")

    cols = st.columns(min(3, len(sugg)))
    for i, q in enumerate(sugg[:3]):
        with cols[i % len(cols)]:
            if st.button(q, use_container_width=True, key=f"sugg_{chat['id']}_{_now_ms()}_{i}"):
                # push it as if user typed it
                chat["messages"].append({"role": "user", "type": "text", "content": q})
                st.rerun()
...
# =============================
# RENDER MESSAGES
# IMPORTANT: user uses chat bubble, assistant uses container (fixes blank blocks)
# =============================
for idx, m in enumerate(chat["messages"]):
    ...
    with st.container():
        ...
        # Executive Insight  ✅ business label stays same
...
""",
        ...
        # Suggested next questions (LLM-based) ✅
        render_suggested_questions(out, chat)
        if (out.get("suggested_questions") or []):
            st.markdown('<div class="hr"></div>', unsafe_allow_html=True)

        # How we calculated it (SQL) ✅ rename
        st.markdown('<div class="card">', unsafe_allow_html=True)
        ...
        # Detailed results ✅ rename + no-rows polish
        st.markdown('<div class="card">', unsafe_allow_html=True)
        ...
        if df.empty:
            ...
            # If backend gave suggestions, show them here too (best place for no-rows UX)
            sugg = out.get("suggested_questions") or []
            sugg = [str(s).strip() for s in sugg if str(s).strip()]
            if sugg:
                ...
                for i, q in enumerate(sugg[:3]):
                    with cols[i % len(cols)]:
                        if st.button(q, use_container_width=True, key=f"no_rows_sugg_{chat['id']}_{idx}_{i}"):
                            chat["messages"].append({"role": "user", "type": "text", "content": q})
                            st.rerun()
            else:
        ...
        # Trend / comparison view ✅ rename (same editable chart)
        st.markdown('<div class="card">', unsafe_allow_html=True)
